package com.choigoyo.Exchange.config.oauth;

import com.choigoyo.Exchange.Entity.UserEntity;
import com.choigoyo.Exchange.config.auth.PrincipalDetails;
import com.choigoyo.Exchange.config.oauth.provider.FaceBookUserInfo;
import com.choigoyo.Exchange.config.oauth.provider.GoogleUserInfo;
import com.choigoyo.Exchange.config.oauth.provider.NaverUserInfo;
...
public class PrincipalOAuth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        String name = provider + "_" + providerId; //  userName 의 중복 방지를 위해서
        ...
        if (userEntity == null) { // userEntity가 null이라면
            userEntity = userEntity.builder() // 생성자를 통해 값을 세팅
                    .name(name)
                    .password(password)
                    .email(email)
                    .role(role)
